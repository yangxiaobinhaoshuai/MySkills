...
按鍵的名稱，大小寫字母和數字都用他們自己表示，其他的按鍵名稱 [參考這裏](https://github.com/rime/librime/blob/master/include/X11/keysymdef.h) [这个更直观的文档](https://github.com/LEOYoon-Tsaw/Rime_collections/blob/master/Rime_description.md) 的定義，去除代碼前綴 `XK_` 即是。
...
patch:
  schema_list:
    ...
    # 以下方案非預裝，請先安裝 [配方包](https://rime.im/recipes/) 再啓用
    - schema: quick5               # 速成
    - schema: jyutping             # 粵拼
    - schema: wubi86               # 五笔86
...
倣此例，可啓用任一預設或自訂輸入方案，如【粵拼】【五筆】【雙拼】等。（詳解：參見前文「定製方案選單」一節）
...
# default.custom.yaml 在全局範圍重定義該組快速鍵，也可定義在 <輸入方案>.custom.yaml
#
...
可知，模板的做法是，在 `speller/algebra` 原有的規則中插入了一些定義模糊音的代碼行。

類似方案如雙拼、粵拼等可參考模板演示的方法改寫 `speller/algebra` 。

#### 【朙月拼音】模糊音配置

【朙月拼音】的配置已經重構，可以更方便地選擇啓用模糊音特性：

https://github.com/rime/rime-luna-pinyin/blob/46acf03142c12b5aeed7002675046bf7255eed35/pinyin.yaml#L9
...
```yaml
  ...
  patch:
    speller/algebra:
      __patch:
        - pinyin:/zh_z_bufen      # 啓用 zh/z 不分
        - pinyin:/n_l_bufen       # 啓用 n/l 不分
        #- pinyin:/r_l_bufen
        #- pinyin:/r_y_bufen
        - pinyin:/hu_f_buhun      # 啓用 hu/f 不昏
        #- pinyin:/eng_ong_bufen
        - pinyin:/en_eng_bufen    # 啓用 en/eng 不分
        #- pinyin:/ziantuan
        #- pinyin:/zhongguan
        - pinyin:/abbreviation          # 啓用簡拼
        - pinyin:/spelling_correction   # 啓用拼寫糾錯
        - pinyin:/key_correction        # 啓用按鍵糾錯
```
...
[`wubi`](https://github.com/rime/rime-wubi) 配方中的 `wubi_trad` 方案已配置好「簡入繁出」，將其加入方案選單即可。
